...
PAGES = [
{
 ...
 "title": "AI 컨택센터(AICC) 구축 | 통화요약·AI 응대·보이는 ARS | 비트웨이브",
 ...
 "answer": "AI 컨택센터는 전화 상담에 <span class='hl'>음성인식과 AI를 붙여</span> 반복되는 문의를 자동으로 처리하고, 통화 내용을 자동으로 정리하는 방식입니다. "
           "비트웨이브는 교환기와 상담 프로그램을 직접 개발하기 때문에, <span class='hl'>기존 시스템을 걷어내지 않고 AI 기능만 추가</span>하는 방식으로 적용합니다.",
 "features": [
   ...
   ("쓰던 시스템 위에", "이미 비트웨이브를 쓰고 계시면 새로 구축하지 않고 기능만 추가합니다."),
 ],
 "targets": [
 ...
 "faq": [
   ("지금 쓰는 시스템을 바꿔야 하나요?", "비트웨이브 시스템을 쓰고 계시면 교체 없이 기능만 추가하는 방식으로 검토합니다. 타사 시스템은 현황을 본 뒤 연동 가능 범위를 알려드립니다."),
...
},
{
 ...
 "title": "콜센터 솔루션 구축 | IP 교환기·호분배·CTI | 비트웨이브",
 ...
 "answer": "콜센터를 돌리려면 전화를 받고 나누는 <b>IP 교환기</b>, 상담원에게 전화를 배분하는 <b>호분배(ACD)</b>, 고객 정보를 띄우는 <b>상담원 화면(CTI)</b>, 그리고 <b>녹취와 통계</b>가 필요합니다. "
           "보통은 이것들을 각각 사서 연동합니다. 비트웨이브는 <span class='hl'>이 전부를 직접 개발해 한 시스템으로 제공</span>하기 때문에 연동 비용과 기간이 들지 않습니다.",
 "features": [
   ...
   ("실시간 전광판", "대기·통화중·후처리·휴식 상태를 한 화면에서 봅니다. 지금 몇 명이 대기 중인지 보입니다."),
...
},
{
 ...
 "title": "상담 CRM 고객관리 프로그램 | 스크린 팝업·상담이력 | 비트웨이브",
 ...
 "answer": "일반 CRM은 사람이 검색해서 고객을 찾습니다. 상담용 CRM은 <span class='hl'>전화가 걸려오는 순간 발신번호로 고객을 자동으로 찾아 화면에 띄웁니다</span>. "
           "통화가 끝나면 상담 내용이 그 고객 이력에 바로 쌓입니다. 비트웨이브 CRM은 교환기와 같은 회사가 만들었기 때문에 전화와 화면이 따로 놀지 않습니다.",
 "features": [
...
},
{
 ...
 "title": "ARS·IVR 자동응답 시스템 구축 | 다단계 시나리오·콜백 | 비트웨이브",
 ...
 "answer": "둘은 거의 같은 뜻으로 쓰입니다. 정확히는 <b>ARS</b>가 녹음된 안내를 들려주는 자동응답이고, <b>IVR</b>은 고객이 누른 번호나 말한 내용에 따라 <span class='hl'>다른 안내로 갈라지는 것</span>까지 포함합니다. "
           "비트웨이브는 단계 수 제한 없이 시나리오를 구성하고, 시간·요일·상황에 따라 다른 안내가 나가도록 설정합니다.",
 "features": [
 ...
 "faq": [
   ...
   ("단계가 너무 많으면 고객이 답답해하지 않나요?", "그래서 보이는 ARS를 함께 권합니다. 음성을 듣지 않고 화면에서 바로 고를 수 있습니다."),
...
},
{
 ...
 "title": "통화 녹취 시스템 | 전수 녹취·조건 검색·권한 관리 | 비트웨이브",
 ...
 "answer": "말로 한 약속을 확인할 방법이 필요하기 때문입니다. 주문 내용이 달라졌다거나 안내를 못 받았다는 이야기가 나올 때, <span class='hl'>녹취가 있으면 확인하면 끝나고 없으면 분쟁이 됩니다</span>. "
           "금융·보험처럼 녹취 보관이 의무인 업종도 있습니다. 비트웨이브는 전 통화 자동 녹취와 조건 검색을 기본으로 제공합니다.",
 "features": [
   ...
   ("보관 위치 선택", "기본은 AWS 클라우드에 보관합니다. 규정상 외부 보관이 어려운 곳은 고객사 서버에 직접 둡니다."),
   ("보관 기간 설정", "얼마나 오래 남길지 정합니다. 업종 규정에 맞춰 조정합니다."),
...
},
{
 ...
 "title": "통합 채팅상담 | 문자·카카오·네이버톡톡 한 화면 | 비트웨이브",
...
},
{
 ...
 "title": "기업 인터넷전화·대표번호 | 070·내선·번호이동 | 비트웨이브",
 ...
 "answer": "인터넷 회선으로 통화하기 때문에 <span class='hl'>회선을 늘리는 데 공사가 필요 없고, 사무실을 옮겨도 번호가 그대로</span>입니다. "
           "내선끼리는 통화료가 들지 않고, 지점이 여러 곳이어도 하나의 내선 체계로 묶입니다. 비트웨이브는 인터넷전화 사업자라 회선을 직접 공급합니다.",
 "features": [
...
},
{
 ...
 "title": "콜센터 부가서비스 | 문자·팩스·전광판·080·안심번호 | 비트웨이브",
 "desc": "필요한 것만 골라 붙이는 부가 기능. 문자 발송, 웹팩스, 실시간 전광판, 080 수신거부, 안심번호, 영상상담 등.",
 ...
 "features": [
   ...
   ("실시간 전광판", "대기·통화중·후처리 상태를 한 화면에 띄웁니다."),
...
},
{
 ...
 "title": "온라인 마케팅 대행 | 문의를 만들고 상담까지 잇습니다 | 비트웨이브",
 ...
 "answer": "20년 동안 상담 시스템을 구축하면서 반복해서 본 장면이 있습니다. <span class='hl'>시스템은 갖췄는데 걸려오는 전화가 없어서 그대로 놀리는 경우</span>입니다. "
           "비트웨이브는 설립 때부터 온라인 마케팅 프로그램 개발과 대행을 함께 해왔습니다. "
 ...
 "faq": [
   ...
   ("비트웨이브 시스템을 안 쓰는데도 되나요?", "마케팅만 따로 진행할 수 있습니다. 다만 상담 시스템까지 함께 쓰시면 성과 확인이 훨씬 정확해집니다."),
...
<header>
  <div class="wrap nav">
    <a href="/" class="logo">Bit<b>WAVE</b></a>
    <nav class="nav-links">
      <div class="has-sub">
    ...
    <div style="display:flex;align-items:center;gap:10px">
      <a href="tel:{TEL_RAW}" class="nav-call"><span>{TEL}<small>평일 09:00 – 18:00</small></span></a>
...
<div class="drawer" id="drawer" onclick="if(event.target===this)this.classList.remove('open')">
  <div class="drawer-panel">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:22px">
      <span class="logo">Bit<b>WAVE</b></span>
...
<footer>
  <div class="wrap">
    <div class="flogo">Bit<b>WAVE</b></div>
    <div class="fnav">
      <a href="/solution/">솔루션</a><a href="/use-cases/">활용사례</a><a href="/industries/">업종별</a>
      <a href="/pricing/">요금</a><a href="/cases/">구축사례</a><a href="/about/">회사소개</a><a href="/contact/">상담문의</a>
    </div>
    <div class="info">
...
def render(p):
    ...
    targets = "".join(f'''
      <div class="step"><span class="sn wn c{i}">{i+1}</span><div><h4>{e(t)}</h4><p>{e(d)}</p></div></div>'''
    ...
    rel = "".join(f'''
      <a href="/solution/{o["slug"]}/" class="ind"><h4>{o["nav"]}</h4><p>{e(o["eyebrow"])}</p></a>'''
      for o in others)
...
def render_hub():
    cards = "".join(f'''
      <a href="/solution/{p["slug"]}/" class="card" style="display:block">
        ...
        <h3>{p["nav"]}</h3>
        <p class="cd">{e(p["desc"].split(".")[0])}.</p>
      </a>''' for p in PAGES)

    return (head("솔루션 전체 | 콜센터·CRM·ARS·녹취·AI | 비트웨이브",
                 "비트웨이브가 제공하는 컨택센터 솔루션 8종. 교환기부터 AI까지 한 회사가 직접 개발합니다.",
                 f"{SITE}/solution/")
+ header() + f'''
...
<div class="hero-top">
  <div class="wrap">
    <div class="hero-center">
      ...
      <h1 style="margin-top:16px;font-size:42px">콜센터에 필요한 전부를<br>한 곳에서 만듭니다</h1>
